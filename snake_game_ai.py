...
class SnakeGameAI:
    ...
    def play_step(self, action):
        ...
        current_speed = min(SPEED + self.score * speed_increment, max_speed)
    ...
    def _move(self, action):
        ...
        if np.array_equal(action, [1, 0, 0]): # if action array = [1, 0, 0]
            new_dir = clock_wise[idx] # no change
        elif np.array_equal(action, [0, 1, 0]): # if action array = [0, 1, 0]
            next_idx = (idx + 1) % 4
